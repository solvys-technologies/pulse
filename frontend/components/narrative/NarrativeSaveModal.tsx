...
export function NarrativeSaveModal({ open, onConfirm, onCancel }: NarrativeSaveModalProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm ${isClosing ? 'animate-fade-out-backdrop' : 'animate-fade-in-backdrop'}`}>
      <div className={`max-w-sm w-full bg-[var(--pulse-bg)] border border-[var(--pulse-accent)]/40 rounded-lg shadow-[0_0_40px_rgba(199,159,74,0.15)] ${isClosing ? 'animate-fade-out' : 'animate-fade-in'}`}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-zinc-900">
          <div className="flex items-center gap-2">
            <Save className="w-4 h-4 text-[var(--pulse-accent)]" />
            <h2 className="text-sm font-bold text-[var(--pulse-accent)]">Save Changes</h2>
          </div>
        ...
        {/* Body */}
        <div className="p-4">
          <p className="text-sm text-[var(--pulse-muted)] leading-relaxed">
        ...
        <div className="flex justify-end gap-3 p-4 border-t border-zinc-900">
          <button
            onClick={handleClose}
            className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800 text-[var(--pulse-text)] rounded-lg text-sm font-medium transition-all"
          >
            Cancel
          </button>
          <button
            onClick={handleConfirm}
            className="px-4 py-2 bg-[var(--pulse-accent)] hover:brightness-110 text-black rounded-lg text-sm font-medium transition-all"
          >
            Save
          </button>
        </div>
