...
function StepIcon({ kind }: { kind: CognitionStepKind }) {
  ...
  return (
    <span className="text-[#c79f4a] font-mono text-[10px] w-3 shrink-0 select-none">
      {glyphs[kind] ?? '·'}
    </span>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <span className="text-[11px] text-[#f0ead6]/80 leading-tight">{step.label}</span>
        {step.detail && (
          <span className="text-[10px] text-[#f0ead6]/40 ml-1.5">{step.detail}</span>
        )}
      </div>
      {step.durationMs !== undefined && (
        <span className="text-[9px] text-[#f0ead6]/25 tabular-nums shrink-0">
          {step.durationMs}ms
        </span>
      )}
    </div>
...
export function CognitionPanel({ requestId, isStreaming }: Props) {
  ...
  return (
    <div
      className="rounded-xl border bg-[#050402]/90 overflow-hidden transition-all"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {isStreaming && !done ? (
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#c79f4a] animate-pulse" />
          ) : (
            <span
              className="inline-block w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: hasError ? '#ef4444' : '#c79f4a', opacity: 0.6 }}
            />
          )}
          <span className="text-[10px] font-medium tracking-wider uppercase text-[#c79f4a]/70">
            Agent Mind
          </span>
        ...
        <span className="text-[#f0ead6]/25 text-[10px] transition-transform" style={{ display: 'inline-block', transform: collapsed ? 'rotate(-90deg)' : 'rotate(0)' }}>
          ▾
        </span>
      </button>
        ...
        <div className="px-3 pb-2.5 space-y-0.5">
          ...
          {isStreaming && !done && (
            <div className="flex items-center gap-2 pt-0.5">
              <span className="text-[#c79f4a] font-mono text-[10px] w-3">·</span>
              <span className="text-[10px] text-[#f0ead6]/30 animate-pulse">processing…</span>
            </div>
          )}
        </div>
      )}
