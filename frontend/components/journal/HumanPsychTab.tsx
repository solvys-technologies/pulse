...
function DisciplineGauge({ score }: { score: number }) {
  ...
  const color = pct >= 80 ? '#34D399' : pct >= 50 ? 'var(--pulse-accent)' : '#EF4444';
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[var(--pulse-surface)] border border-[var(--pulse-accent)]/10 rounded-lg p-3">
        <div className="flex items-center gap-2 mb-2">
          <TrendingDown className="w-3.5 h-3.5 text-[var(--pulse-accent)]" />
          <span className="text-xs font-semibold text-[var(--pulse-text)]">ER Trend</span>
          {er?.isMonitoring && (
        ...
        {combinedTrend.length >= 2 ? (
          ...
          <div className="text-[10px] text-[var(--pulse-muted)] h-10 flex items-center">
            Start monitoring to see ER trend
          </div>
        )}
        {er && (
          <div className="flex items-center justify-between mt-1.5 text-[10px]">
            <span className="text-[var(--pulse-muted)]">Current</span>
      ...
      <div className="bg-[var(--pulse-surface)] border border-[var(--pulse-accent)]/10 rounded-lg p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-semibold text-[var(--pulse-text)]">Infractions</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <div className="text-[10px] text-[var(--pulse-muted)]">Today</div>
            <div className="text-lg font-mono text-[var(--pulse-text)]">
              {er?.infractionCount ?? todayEntry?.infractions?.length ?? 0}
            </div>
          </div>
          <div>
            <div className="text-[10px] text-[var(--pulse-muted)]">This Week</div>
            <div className="text-lg font-mono text-[var(--pulse-text)]">{weekInfractions}</div>
          </div>
        </div>
      ...
      <div className="bg-[var(--pulse-surface)] border border-[var(--pulse-accent)]/10 rounded-lg p-3">
        <div className="flex items-center gap-2 mb-2">
          <Shield className="w-3.5 h-3.5 text-[var(--pulse-accent)]" />
          <span className="text-xs font-semibold text-[var(--pulse-text)]">Discipline</span>
        </div>
        <DisciplineGauge score={todayEntry?.disciplineScore ?? avgDiscipline} />
        <div className="text-[10px] text-[var(--pulse-muted)] mt-1.5">
      ...
      {/* Session Notes */}
      <div className="bg-[var(--pulse-surface)] border border-[var(--pulse-accent)]/10 rounded-lg p-3">
        <div className="text-xs font-semibold text-[var(--pulse-text)] mb-2">Session Notes</div>
        <textarea
          value={notes}
          onChange={e => setNotes(e.target.value)}
          placeholder="What happened today? How did you feel about your trades?"
          className="w-full h-20 bg-black/30 border border-[var(--pulse-accent)]/10 rounded p-2 text-xs text-[var(--pulse-text)] placeholder:text-[var(--pulse-muted)] resize-none focus:outline-none focus:border-[var(--pulse-accent)]/30"
        />
      </div>
      ...
      <div className="bg-[var(--pulse-surface)] border border-[var(--pulse-accent)]/10 rounded-lg p-3">
        <div className="flex items-center gap-2 mb-2">
          <Heart className="w-3.5 h-3.5 text-[var(--pulse-accent)]" />
          <span className="text-xs font-semibold text-[var(--pulse-text)]">Emotional Control</span>
          {erDerivedScore && (
            <span className="text-[9px] text-[var(--pulse-muted)] ml-auto">
              ER score: <span className="font-mono text-[var(--pulse-accent)]">{erDerivedScore}/10</span>
            </span>
          )}
        </div>
          ...
          {Array.from({ length: 10 }, (_, i) => i + 1).map(n => (
            <button
              ...
              className={`flex-1 py-1.5 rounded text-[10px] font-mono transition-all ${
                emotionalRating === n
                  ? 'bg-[var(--pulse-accent)] text-black font-semibold'
                  : 'bg-black/30 border border-[var(--pulse-accent)]/10 text-[var(--pulse-muted)] hover:border-[var(--pulse-accent)]/30 hover:text-[var(--pulse-text)]'
              }`}
            >
              {n}
            </button>
        ...
        <div className="flex justify-between mt-1 text-[9px] text-[var(--pulse-muted)]">
          <span>Tilted</span>
          <span>Composed</span>
        </div>
      </div>
      ...
      <button
        ...
        disabled={saving}
        className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-[11px] font-medium bg-[var(--pulse-accent)] text-black rounded hover:brightness-110 active:scale-95 transition-all disabled:opacity-50"
      >
      ...
      {/* Blindspots */}
      <div className="bg-[var(--pulse-surface)] border border-[var(--pulse-accent)]/20 rounded-lg p-3">
        <div className="flex items-center gap-2 mb-2">
          <Eye className="w-3.5 h-3.5 text-[var(--pulse-accent)]" />
          <span className="text-xs font-semibold text-[var(--pulse-text)]">Blindspots</span>
          {blindspotSource && blindspotSource !== 'error' && blindspotSource !== 'empty' && (
            <span className="text-[9px] text-[var(--pulse-muted)] ml-auto">via {blindspotSource}</span>
          )}
        </div>
        {blindspots.length > 0 ? (
              ...
              <div
                ...
                className={`flex items-start gap-2 p-2 rounded border ${
                  spot.severity === 'high'
                    ...
                    : 'bg-[var(--pulse-accent)]/5 border-[var(--pulse-accent)]/20'
                }`}
              >
                <AlertTriangle className={`w-3 h-3 mt-0.5 flex-shrink-0 ${
                  spot.severity === 'high' ? 'text-red-400' : 'text-[var(--pulse-accent)]'
                }`} />
                <span className="text-[10px] text-[var(--pulse-text)] leading-tight">{spot.text}</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="text-[10px] text-[var(--pulse-muted)] py-3 text-center">
